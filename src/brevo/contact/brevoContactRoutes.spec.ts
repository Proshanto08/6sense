...
import { createContact, getContactById, updateContact, deleteContact, getAllContacts } from './brevoContactService';
...
describe('Contacts Routes', () => {
  describe('POST /contacts', () => {
    it('should create a contact', async () => {
      const mockContact = { email: 'test@example.com', attributes: {}, listIds: [], updateEnabled: true };
      ...
      expect(createContact).toHaveBeenCalledWith(mockContact.email, mockContact.attributes, mockContact.listIds, mockContact.updateEnabled);
    });
  });
  ...
  describe('GET /contacts', () => {
    it('should get all contacts', async () => {
      ...
      expect(response.body).toEqual(mockResult);
      expect(getAllContacts).toHaveBeenCalledWith(undefined, undefined, 'desc'); // default values
    });
  });
  ...
  describe('PUT /contacts/:identifier', () => {
    it('should update a contact', async () => {
      const mockUpdate = { email: 'updated@example.com', attributes: {}, listIds: [], updateEnabled: false };
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual(mockResult);
      expect(updateContact).toHaveBeenCalledWith('123', mockUpdate.email, mockUpdate.attributes, mockUpdate.listIds, mockUpdate.updateEnabled);
    });
  });
