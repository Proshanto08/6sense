  ...
  describe('sendContactEmail', () => {
    ...
    it('should send a contact email successfully', async () => {
      ...
      const sanitizedHtmlContent = sanitizeHtml(`
        <html>
          <body>
            ...
            <p><strong>Name:</strong> ${sanitizeHtml(contactProperties.name)}</p>
            <p><strong>Business Email:</strong> ${sanitizeHtml(contactProperties.email)}</p>
            <p><strong>Company Website:</strong> ${sanitizeHtml(contactProperties.companyWebsite || 'N/A')}</p>
            <p><strong>Message/Project Brief:</strong> ${sanitizeHtml(contactProperties.message || 'N/A')}</p>
            <p><strong>Get an NDA:</strong> ${contactProperties.getNda ? 'true' : 'false'}</p>
            <p><strong>Consent to Data Processing:</strong> ${contactProperties.consent ? 'true' : 'false'}</p>
          </body>
        </html>
      `);
      ...
      expect(mockedApiRequest).toHaveBeenCalledWith(
        ...
        'Email successfully sent',
        {
          subject: `New Contact Form Submission from ${sanitizeHtml(contactProperties.name)}`,
