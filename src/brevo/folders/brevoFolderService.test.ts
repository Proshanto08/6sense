...
const mockBrevoClientError = (method: 'get' | 'post' | 'put' | 'delete', errorMessage: string) => {
  (mockedBrevoClient[method] as jest.Mock).mockRejectedValue({ message: errorMessage });
};

const checkResponse = (result: IApiResponse, expectedStatus: number, expectedMessage: string) => {
...
describe('Folder Service', () => {
  describe('createFolder', () => {
    it('should create a folder successfully', async () => {
      ...
      expect(mockedBrevoClient.post).toHaveBeenCalledWith('/contacts/folders', { name: folderName });
    });
  ...
  describe('getFolder', () => {
    it('should retrieve a folder successfully', async () => {
      ...
      expect(mockedBrevoClient.get).toHaveBeenCalledWith(`/contacts/folders/${folderId}`);
    });
  ...
  describe('updateFolder', () => {
    it('should update a folder successfully', async () => {
      ...
      expect(mockedBrevoClient.put).toHaveBeenCalledWith(`/contacts/folders/${folderId}`, { name: folderName });
    });
  ...
  describe('deleteFolder', () => {
    it('should delete a folder successfully', async () => {
      ...
      expect(mockedBrevoClient.delete).toHaveBeenCalledWith(`/contacts/folders/${folderId}`);
    });
  ...
  describe('getFolderLists', () => {
    it('should retrieve lists from a folder successfully', async () => {
      ...
      expect(mockedBrevoClient.get).toHaveBeenCalledWith(`/contacts/folders/${folderId}/lists`, {
        params: { limit: undefined, offset: undefined, sort: undefined },
      });
    });
