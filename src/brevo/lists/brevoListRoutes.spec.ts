...
describe('Lists Routes', () => {
  ...
  describe('GET /lists', () => {
    it('should get all lists', async () => {
      const mockResult = { status: 200, data: [{ id: '456', name: 'List 1', folderId: '123' }] };
      ...
      expect(response.body).toEqual(mockResult);
      expect(getAllLists).toHaveBeenCalledWith(undefined, undefined, 'desc'); // Default values
    });
  });
  ...
  describe('PUT /lists/:listId', () => {
    it('should update a list', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual(mockResult);
      expect(updateList).toHaveBeenCalledWith(456, mockUpdate.name, mockUpdate.folderId);
    });
  });
  ...
  describe('GET /contacts/lists/:listId/contacts', () => {
    it('should get contacts from a list', async () => {
      const mockResult = { status: 200, data: [{ id: '789', email: 'contact@example.com' }] };
      const queryParams = { modifiedSince: '2024-01-01T00:00:00Z', limit: '10', offset: '0', sort: 'asc' };

      (getContactsFromList as jest.Mock).mockResolvedValue(mockResult);

      const response = await request(app).get('/contacts/lists/456/contacts').query(queryParams);

      expect(response.status).toBe(200);
      expect(response.body).toEqual(mockResult);
      expect(getContactsFromList).toHaveBeenCalledWith(456, '2024-01-01T00:00:00Z', 10, 0, 'asc');
    });
  });
  ...
  describe('POST /contacts/lists/:listId/contacts/add', () => {
    it('should add contacts to a list', async () => {
      ...
      const response = await request(app).post('/contacts/lists/456/contacts/add').send({ emails: mockEmails });
