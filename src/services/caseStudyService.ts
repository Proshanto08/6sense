...
export const getBasicProjects = async (page: number = 1, limit: number = 6): Promise<IApiResponse> => {
  try {
...
export const updateProject = async (slug: string, updateData: Partial<IProject>): Promise<IApiResponse> => {
  try {
    if (updateData.appName) {
      updateData.slug = slugify(updateData.appName, { lower: true, strict: true });
    }
